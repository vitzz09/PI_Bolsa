...
def carregar_bcb(indicador: str, meses: int) -> pd.DataFrame | None:
    ...
    try:
        ...
        df = df[["data", "valor"]].sort_values("data").reset_index(drop=True)
        return df
    except Exception:
        return None
...
def get_serie(indicador: str, meses: int) -> tuple[pd.DataFrame, bool]:
    """Retorna (DataFrame, is_real). Tenta BCB, cai em simulado."""
    df = carregar_bcb(indicador, meses)
    if df is not None and len(df) >= 6:
        # Cortar para o número de meses solicitado
...
def prever(df_hist: pd.DataFrame, meses_pred: int, algo: str) -> dict:
    ...
    # Datas futuras
    ultima_data  = df_hist["data"].iloc[-1]
...
with st.sidebar:
    ...
    Tenta carregar dados <b style="color:#10b981">reais</b> do
    <b>Banco Central do Brasil</b> (SGS).<br>
    Se indisponível, usa série simulada coerente
...
if st.session_state.res:
    ...
    if len(ind_sel) > 1:
        ...
        fig2.add_vline(
            x=resultados[ind_sel[0]]["historico"]["data"].iloc[-1],
            line_dash="dash", line_color="#334155",
